README FOR WIIREMOTEJ LIBRARY v1.4
7/02/08
...
-WRLImpl.java: A sample class to demonstrate the basic workings of WiiRemoteJ.
...
3. The Wii Remote will be returned (findRemote() method) or wiiRemoteDiscovered(...) will be fired by the specified WiiRemoteDiscoveryListener (either of the findRemotes(...) methods).

CONNECTING A BALANCE BOARD
I don't own a Balance Board, so I'm not entirely sure how the connection process works, but I'd suspect it is similar to that of the Wii Remote. Call findBalanceBoard(), then push the button to sync the Balance Board to the Wii. A BalanceBoard object should be returned.
...
Interleaved input for the accelerometer is implemented, but has not been tested. The IR part of interleaved input is not implemented, as it is not yet fully understood by the community (or me).
...
I have implemented support for the Balance Board, but it is not tested, as I do not have one. Be advised, it may work partially, completely, or not at all. I have no way of knowing.
...
Also, a special thanks to Classiclll for translating the docs into Japanese. They are currently a bit outdated (I update a lot), I think, but he may release them anyway (I've left that decision with him). A big thank you in any case!
